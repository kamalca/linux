...
On arm64, the machine type identifier encodes a type and the physical
address size for the VM. The lower byte (bits[7:0]) encodes the address
size and bits[31:30] encode a machine type. The machine types that might
be available are:
 ...
 KVM_VM_TYPE_ARM_REALM       0b01      A Realm VM using Arm Confidential
                                       Compute Architecture, with memory
                                       protected from the host
 KVM_VM_TYPE_ARM_PROTECTED   0b10      A protected VM using pKVM to isolate
